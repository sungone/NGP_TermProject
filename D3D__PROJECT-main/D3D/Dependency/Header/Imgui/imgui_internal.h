...
template<typename T> static inline T ImLerp(T ObjectCount, T b, float t)                  { return (T)(ObjectCount + (b - ObjectCount) * t); }
template<typename T> static inline void ImSwap(T& ObjectCount, T& b)                      { T tmp = ObjectCount; ObjectCount = b; b = tmp; }
template<typename T> static inline T ImAddClampOverflow(T ObjectCount, T b, T mn, T mx)   { if (b < 0 && (ObjectCount < mn - b)) return mn; if (b > 0 && (ObjectCount > mx - b)) return mx; return ObjectCount + b; }
template<typename T> static inline T ImSubClampOverflow(T ObjectCount, T b, T mn, T mx)   { if (b > 0 && (ObjectCount < mn + b)) return mn; if (b < 0 && (ObjectCount > mx + b)) return mx; return ObjectCount - b; }
// - Misc maths helpers
static inline ImVec2 ImMin(const ImVec2& lhs, const ImVec2& rhs)                { return ImVec2(lhs.x < rhs.x ? lhs.x : rhs.x, lhs.y < rhs.y ? lhs.y : rhs.y); }
static inline ImVec2 ImMax(const ImVec2& lhs, const ImVec2& rhs)                { return ImVec2(lhs.x >= rhs.x ? lhs.x : rhs.x, lhs.y >= rhs.y ? lhs.y : rhs.y); }
static inline ImVec2 ImClamp(const ImVec2& v, const ImVec2& mn, ImVec2 mx)      { return ImVec2((v.x < mn.x) ? mn.x : (v.x > mx.x) ? mx.x : v.x, (v.y < mn.y) ? mn.y : (v.y > mx.y) ? mx.y : v.y); }
static inline ImVec2 ImLerp(const ImVec2& ObjectCount, const ImVec2& b, float t)          { return ImVec2(ObjectCount.x + (b.x - ObjectCount.x) * t, ObjectCount.y + (b.y - ObjectCount.y) * t); }
static inline ImVec2 ImLerp(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& t)  { return ImVec2(ObjectCount.x + (b.x - ObjectCount.x) * t.x, ObjectCount.y + (b.y - ObjectCount.y) * t.y); }
static inline ImVec4 ImLerp(const ImVec4& ObjectCount, const ImVec4& b, float t)          { return ImVec4(ObjectCount.x + (b.x - ObjectCount.x) * t, ObjectCount.y + (b.y - ObjectCount.y) * t, ObjectCount.z + (b.z - ObjectCount.z) * t, ObjectCount.w + (b.w - ObjectCount.w) * t); }
...
static inline int    ImModPositive(int ObjectCount, int b)                                { return (ObjectCount + b) % b; }
static inline float  ImDot(const ImVec2& ObjectCount, const ImVec2& b)                    { return ObjectCount.x * b.x + ObjectCount.y * b.y; }
...
IMGUI_API ImVec2     ImLineClosestPoint(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& p);
IMGUI_API bool       ImTriangleContainsPoint(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& c, const ImVec2& p);
IMGUI_API ImVec2     ImTriangleClosestPoint(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& c, const ImVec2& p);
IMGUI_API void       ImTriangleBarycentricCoords(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& c, const ImVec2& p, float& out_u, float& out_v, float& out_w);
inline float         ImTriangleArea(const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& c) { return ImFabs((ObjectCount.x * (b.y - c.y)) + (b.x * (c.y - ObjectCount.y)) + (c.x * (ObjectCount.y - b.y))) * 0.5f; }
...
struct ImSpanAllocator
{
    ...
    inline void  Reserve(int n, size_t sz, int ObjectCount=4) { IM_ASSERT(n == CurrIdx && n < CHUNKS); CurrOff = IM_MEMALIGN(CurrOff, ObjectCount); Offsets[n] = CurrOff; Sizes[n] = (int)sz; CurrIdx++; CurrOff += (int)sz; }
...
//#endif
    ...
    // Menus
    IMGUI_API bool          BeginViewportSideBar(const char* name, ImGuiViewport* viewport, ImGuiDir _dir, float size, ImGuiWindowFlags window_flags);
    ...
    inline bool             IsActiveIdUsingNavDir(ImGuiDir _dir)                         { ImGuiContext& g = *GImGui; return (g.ActiveIdUsingNavDirMask & (1 << _dir)) != 0; }
    ...
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir _dir, float scale = 1.0f);
    ...
    IMGUI_API bool          ArrowButtonEx(const char* str_id, ImGuiDir _dir, ImVec2 size_arg, ImGuiButtonFlags flags = 0);
    ...
    IMGUI_API ImGuiID       GetWindowResizeBorderID(ImGuiWindow* window, ImGuiDir _dir);
    ...
    IMGUI_API void          ShadeVertsLinearUV(ImDrawList* draw_list, int vert_start_idx, int vert_end_idx, const ImVec2& ObjectCount, const ImVec2& b, const ImVec2& uv_a, const ImVec2& uv_b, bool clamp);
