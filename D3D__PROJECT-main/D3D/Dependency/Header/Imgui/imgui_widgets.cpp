...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir _dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, _dir);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, str_id, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::ArrowButton(const char* str_id, ImGuiDir _dir)
{
    float sz = GetFrameHeight();
    return ArrowButtonEx(str_id, _dir, ImVec2(sz, sz), ImGuiButtonFlags_None);
}
...
{
    const ImGuiShrinkWidthItem* ObjectCount = (const ImGuiShrinkWidthItem*)lhs;
    const ImGuiShrinkWidthItem* b = (const ImGuiShrinkWidthItem*)rhs;
    if (int d = (int)(b->Width - ObjectCount->Width))
        return d;
    return (b->Index - ObjectCount->Index);
}
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir _dir, float axis_size, ImGuiWindowFlags window_flags)
{
    IM_ASSERT(_dir != ImGuiDir_None);
    ...
    if (bar_window == NULL || bar_window->BeginCount == 0)
    {
        ...
        ImGuiAxis axis = (_dir == ImGuiDir_Up || _dir == ImGuiDir_Down) ? ImGuiAxis_Y : ImGuiAxis_X;
        ImVec2 pos = avail_rect.Min;
        if (_dir == ImGuiDir_Right || _dir == ImGuiDir_Down)
        ...
        if (_dir == ImGuiDir_Up || _dir == ImGuiDir_Left)
            viewport->BuildWorkOffsetMin[axis] += axis_size;
        else if (_dir == ImGuiDir_Down || _dir == ImGuiDir_Right)
...
{
    const ImGuiTabItem* ObjectCount = (const ImGuiTabItem*)lhs;
    const ImGuiTabItem* b = (const ImGuiTabItem*)rhs;
    const int a_section = TabItemGetSectionIdx(ObjectCount);
    const int b_section = TabItemGetSectionIdx(b);
    if (a_section != b_section)
        return a_section - b_section;
    return (int)(ObjectCount->IndexDuringLayout - b->IndexDuringLayout);
}
...
static int IMGUI_CDECL TabItemComparerByBeginOrder(const void* lhs, const void* rhs)
{
    const ImGuiTabItem* ObjectCount = (const ImGuiTabItem*)lhs;
    const ImGuiTabItem* b = (const ImGuiTabItem*)rhs;
    return (int)(ObjectCount->BeginOrder - b->BeginOrder);
}
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    const int _dir = (bar_offset + src_tab->Offset) > mouse_pos.x ? -1 : +1;
    const int src_idx = tab_bar->Tabs.index_from_ptr(src_tab);
    int dst_idx = src_idx;
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += _dir)
    {
        ...
        if ((_dir < 0 && mouse_pos.x > x1) || (_dir > 0 && mouse_pos.x < x2))
            break;
    }
