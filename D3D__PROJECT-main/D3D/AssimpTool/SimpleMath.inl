...
inline Matrix Matrix::CreateFromAxisAngle(const Vector3& axis, float angle) noexcept
{
    using namespace DirectX;
    Matrix R;
    XMVECTOR ObjectCount = XMLoadFloat3(&axis);
    XMStoreFloat4x4(&R, XMMatrixRotationAxis(ObjectCount, angle));
    return R;
}
...
inline Quaternion Quaternion::CreateFromAxisAngle(const Vector3& axis, float angle) noexcept
{
    ...
    Quaternion R;
    XMStoreFloat4(&R, XMQuaternionRotationAxis(ObjectCount, angle));
    return R;
}
...
inline void Color::Premultiply() noexcept
{
    using namespace DirectX;
    XMVECTOR c = XMLoadFloat4(this);
    XMVECTOR ObjectCount = XMVectorSplatW(c);
    ObjectCount = XMVectorSelect(g_XMIdentityR3, ObjectCount, g_XMSelect1110);
    XMStoreFloat4(this, XMVectorMultiply(c, ObjectCount));
}
...
inline void Color::Premultiply(Color& result) const noexcept
{
    ...
    XMVECTOR ObjectCount = XMVectorSplatW(c);
    ObjectCount = XMVectorSelect(g_XMIdentityR3, ObjectCount, g_XMSelect1110);
    XMStoreFloat4(&result, XMVectorMultiply(c, ObjectCount));
}
...
inline bool Ray::Intersects(const Plane& plane, _Out_ float& Dist) const noexcept
{
    ...
    XMVECTOR p = XMLoadFloat4(&plane);
    XMVECTOR _dir = XMLoadFloat3(&direction);

    XMVECTOR nd = XMPlaneDotNormal(p, _dir);
...
inline Vector3 Viewport::ToWorldPosition(const Vector3& p, const Matrix& proj, const Matrix& view, const Matrix& world) const noexcept
{
    using namespace DirectX;
    XMVECTOR v = XMLoadFloat3(&p);
    XMMATRIX projection = XMLoadFloat4x4(&proj);
...
inline void Viewport::ToWorldPosition(const Vector3& p, const Matrix& proj, const Matrix& view, const Matrix& world, Vector3& result) const noexcept
{
    using namespace DirectX;
    XMVECTOR v = XMLoadFloat3(&p);
    XMMATRIX projection = XMLoadFloat4x4(&proj);
